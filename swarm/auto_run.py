...
def main():
    ...
    output_folder = os.path.join(script_dir, 'output/2024-11')
   ...
   # Initialize an empty list to store combined data from all files
    combined_data = []
    ...
    # Check if combined data is non-empty
    if combined_data:
        print(f"Total records before sorting: {len(combined_data)}")

        # Sort the combined data by category and usefulness
        combined_data.sort(key=lambda x: (str(x.get('report_category', 'Unknown')),
                                        str(x.get('periodic', 'Unknown'))))

        # Save all combined and sorted entries to the output file as an HTML table
    ...
    for index in range(num_start, num_end):
        ...
        try:
            ...
            metadata_html = f"<head><title>November {table_metadata.get('item_noun', 'Item')}s</title></head><body><h1>November update for {table_metadata.get('item_noun', 'Item')}s</h1>\n<h1>Table Metadata</h1>"
            ...
            for column in column_metadata:
                # Display all fields from the column metadata
                row = "<tr>"
                if isinstance(column, dict):
                    for key, value in column.items():
                        row += f"<td>{value}</td>"
                else:
                    # Handle string case by just outputting the value
                    row += f"<td>{column}</td>"
                    continue
                row += "</tr>"
                metadata_html += row
          ...
          # Prepare Markdown metadata using the same fields as HTML
            ...
            for column in column_metadata:
                if isinstance(column, dict):
                    ...
                    # If column is a string, just put it in one cell
